...
This section of the Appendix aims to provide a minimum working example of the commands necessary to reduce \polsalt\ data using \stops\ and \iraf. It contains the commands necessary to activate all software and run through the reduction process but makes no attempt at discussion.\footnote{Please see \S~\ref{sec:red_proc} for a discussion of the commands and default `\texttt{<...>}' parameters.}
\prgph
...
\begin{figure}[h!]
    ...
    \end{minipage}
    \caption{The typical minimal file structure of data provided by \gls{SALT}.\protect\footnotemark}
    \label{dir:pre_red}
\end{figure}
\footnotetext{Data located in the `sci' folder is often provided by \gls{SALT} but does not form part of the minimal file structure necessary to begin the reduction process.}
\prgph
