...
% From these matrices and the chain rule in Equation~\ref{eq:mueller_train} we can find that a resultant Stokes vector $\matr{S}^{\prime}$ for a setup similar to Figure~\ref{fig:polarimeter} is given by:

% \begin{gather}
    %     \matr{S}^{\prime} = \matr{P} \matr{W} \matr{S} \nonumber \\
    %     \therefore \matr{S}^{\prime} = \frac{1}{2} \left[ I + Q \cos2\beta + U \cos\alpha \sin2\beta + V \sin\alpha \sin2\beta\right]
    % \end{gather}
