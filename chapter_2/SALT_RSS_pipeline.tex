...
What is measured on the detector by each exposure are the wavelength dependent intensities of the $O$- and $E$-beams, $f_{O}(\lambda)$ and $f_{E}(\lambda)$, respectively, for varying half wave-plate angles, $\theta_{i}$. The $O$- and $E$-beam intensities thus relate to the Stokes parameters following:

\begin{equation}
  \begin{gathered}
    f_{O, i}(\lambda) = \frac{1}{2} [I + Q\cos(4\theta_{i}) + U\sin(4\theta_{i})], \\
    f_{E, i}(\lambda) = \frac{1}{2} [I - Q\cos(4\theta_{i}) - U\sin(4\theta_{i})].
  \end{gathered}
...
\noindent which then allows Equation~\ref{eq:specpol_flux_to_Stokes} to be written in terms of the normalized Stokes parameters, $\bar{Q} = Q / I$ and $\bar{U} = U / I$, or, alternatively, the degree of polarization, $P$, and polarization angle, $\chi$:
...
The desired degree of polarization as well as the polarization angle for linear spectropolarimetry is calculated as:
...
\subsection{The Robert Stobie Spectrograph} \label{subsec:RSS}
...
\section{RSS Spectropolarimetric Reductions} \label{sec:RSS_reductions} % In depth

\todo{How spectropolarimetry reductions differ from spectro\-scopy, polar\-imetry, and how general spectro\-polarimetry reductions are done}
