...
Multi-channel \gls{CCD}'s are detectors that use either multiple \gls{CCD}'s or a \gls{CCD} with multiple output amplifiers which can be read out through multiple channels at the same time, increasing the detectors size while keeping the readout time constant. These \gls{CCD} setups need additional calibrations, specifically cross-talk corrections and mosaicking.
\prgph
...
The wavelength calibrations then consist of identifying a \gls{2D} pixel to wavelength conversion function from the arc frame which may later be applied to calibrate the science frames. The two most common \gls{2D} approximations for wavelength calibrations are the Chebyshev and Legendre polynomial approximations as found in Equations \ref{eq:chebyshev} and \ref{eq:legendre}, respectively. The Chebyshev polynomials are defined axplicitly as:

\begin{gather}
    T_{0}(x) = 1 \nonumber \\
    T_{1}(x) = x \label{eq:chebypoly} \\
    T_{n + 1}(x) = 2 x T_{n}(x) - T_{n - 1}(x), \text{ for } n \geq 1 \nonumber
\end{gather}
...
\begin{equation}
    ...
    \label{eq:chebypolyapprox}
\end{equation}

\noindent where $T$ is a Chebyshev polynomial\footnote{Denoted $T$ as a hold-over from the alternate spelling, `Tchebycheff'.} of degree $n$. These Chebyshev polynomials are a set of orthogonal polynomials, when weighted by $1 / \sqrt{1 - x^{2}}$, meaning that the inner product of any two different Chebyshev polynomials, $T_{i}(x)$ and $T_{j}(x)$, over the range of $[-1, 1]$ is zero.
...
The unknown calibration function may then be approximated by Chebyshev polynomials using:

\begin{equation}
    f(x) \approx \sum_{k = 0}^{N}  c_{k} T_{k}(u)
    \label{eq:chebyshev}
\end{equation}

\noindent where $N$ is the desired order, and $u \in [-1, 1]$ is a scaling factor to remap the variable $x \in [a, b]$ so that the orthogonality of the Chebyshev polynomials holds true.

\begin{align}
    u &= \frac{2 x - a - b}{b - a} & x &= \frac{b - a}{2} u + \frac{a+b}{2}
    \label{eq:chebyXtoU}
\end{align}
...
\begin{equation}
    c_{i} = \frac{2}{N} \sum_{k = 0}^{N - 1} f(x_{k}) T_{i}(x_{k})
    \label{eq:chebycoeff}
\end{equation}

The Chebyshev polynomials are more ideally suited for wavelength calibrations than standard polynomials since they are orthogonal and have minima and maxima located at $[-1, 1]$, as seen in Figure \ref{fig:chebyshev}. This means that the chebyshev approximation is exact when $x$ is located at the x-intercepts of $T_{N}(x)$ which greatly minimizes the error in the approximation, even at lower order approximations \citep{cheby}.
\prgph
...
The Legendre polynomial is defined as:
...
TODO: Wollaston corrections and Tilt corrections
