...
\textsc{polsalt} refers to software developed by Ken Nordsieck and Enrico Kotze with the intended purpose of having a complete reduction pipeline for spectropolarimetric data taken using \glspl{SALT} \gls{RSS}. Newer versions of the software include a \gls{GUI} as well as limited interactivity during key steps in the reduction process, and it is this beta version for which the reduction process is outlined\footnote{\textsc{polsalt} is made freely available via the \textsc{polsalt} GitHub repository, available at \url{https://github.com/saltastro/polsalt}. Great care must be taken during installation}.
\prgph

The steps that make up the reduction pipeline include basic \gls{CCD} reductions, wavelength calibrations, background subtraction and spectral extraction, raw stokes calculations, final stokes calculations, and visualization of the results. As the \textsc{polsalt} GitHub repository and its documentation is available publicly, a general reduction process will be outlined, noting which processes are run.

\paragraph{Basic \gls{CCD} reductions} Basic \gls{CCD} reductions use \textbf{imred.py} and apply basic reductions to the raw data necessary before any calibrations may be done. These corrections include overscan subtractions, gain corrections, crosstalk corrections, and mosaicking as well as attaching the bad pixel maps and variance information. Files with basic reductions performed have ``mxgbp" prepended to their names, and as of February 2022, this reduction step is not required as data received from \gls{SALT} has these files as included as product data.

\paragraph{Wavelength calibrations} Wavelength calibrations use \textbf{specpolwavmap.py} and separately calibrate the $O$- and $E$-beams for all necessary frames before applying cosmic-ray rejections. This step is interactive and allows the user to individually fit wavelength calibration maps to each beam. The importance of good correlation between both beams has been touched on previously and will be further discussed in \S~\ref{sec:polsalt_limits}. The wavelength calibrated results are saved to files with ``w" prepended to their names, and the $O$- and $E$-beams of the extensions within each file are split into their own sub-extensions.
...
The files generated containing the raw stokes information have a very specific naming style, with most notably the pair of frames used being included.

\paragraph{Final stokes calculations} The Final stokes calculations use \textbf{specpolfinalstokes.py} and, using the waveplate pattern along with the raw stokes signals, calibrates for polarimetric zero-point and waveplate efficiency calibrations and calculates the final stokes parameters. The raw stokes are culled using a systematic error as well as a chi-squared analysis.

\paragraph{Visualization} Plotting the results of the spectropolarimetric reduction process uses \textbf{specpolview.py} and generates a plot of the Intensity, Linear Polarization ($\%$), and Equatorial Polarization Angle ($\deg$) against a shared wavelength axis, as seen in Figure~\ref{fig:polsalt_plot}. Multiple options allow for variations in the resultant plots, for example the axis ranges, binning, etc.
...
\paragraph{Post-processing analysis} Generally, the plot of the spectropolarimetric results is the stopping point for most reduction procedures as it contains or creates the desired results. However, two additional tools exist which may be used after the visualization has been completed and are not represented in the \gls{GUI}, namely, flux calibrations and synthetic filtering.

Flux-calibrations use \textbf{specpolflux.py} and are only intended for shape corrections of the spectrum due to the unique design of \gls{SALT}. Additionally, the flux database file must exist for the standard observed and must be copied over to the working science directory.

Synthetic filtering uses \textbf{specpolfilter.py} and computes the synthetically filtered polarization results. The filters which can be synthesized are the Johnson $U$, $B$, and $V$ filter curves from the \gls{SALTICAM} filters, as well as the Cousins $R$, and $I$ filter curves, along with any user defined wavelength dependent throughput.
