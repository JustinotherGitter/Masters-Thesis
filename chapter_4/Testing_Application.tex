...
The \stops\ \texttt{split} method requires any \polsalt\ pre-reduced (`mxgbp-' prefixed) \gls{FITS} files as input and outputs \iraf\ compatible file structures. As no `split' \gls{FITS} files are created during pure \polsalt\ reductions, the \stops\ \texttt{split} method was tested by comparing the pre-reduced \polsalt\ files to the \texttt{split} methods output files, ensuring the correct structure and data integrity of the files handed off to \iraf.
...
\autoref{table:join_info} shows the \gls{FITS} file information for the files with both \polsalt\ and \iraf\ wavelength calibrations. %The joined `wbeam*.fits' files contain the joined \gls{SCI} extension data as well as the same header as the pre-reduced file. The only changes to the data of each file is the addition of the wavelength solution data, which is appended to the end of the \gls{SCI} extension data. This accounts for the discrepancy in the `Dimensions' between the input and output files in \autoref{table:join_info}.

% The \stops\ \texttt{join} method was tested to ensure compatibility and correctness of the output data in comparison to \polsalt. This involved:
% \iraf\ to \polsalt\ file structure conversion
% Matching the joined output with \polsalt\ outputs.
% Show changes to data files are intended (I.E. only joining the data, and `WAV' appended. No changes to the data itself)



% \begin{itemize}
%     \item Ensuring the Wollaston correction was accurately applied to wavelength and bpm extensions.
