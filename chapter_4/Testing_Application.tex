...
As discussed in \cite[][see also \autoref{app:papers}]{cooper_HEASA2022}, as well as expanding on the work presented in \cite{cooper_HEASA2021}, the \stops\ pipeline was used to reduce the blazar 3C~$279$ and the spectropolarimetric standard star, Hiltner-$652$, as observed on the dates tabulated in \autoref{table:sci_targets}. Presented are the observations of 3C~$279$ with grating and articulation angles matching those of the spectrophotometric standard star, Hiltner-$600$, such that the intensity spectra could be relatively flux calibrated, as well as an observation of Hiltner-$652$ for Stokes parameter comparisons.

\begin{figure}[t]
    \centering
    \includegraphics[width = 1.0\textwidth]{4_3C279_all.pdf}
    \caption{Spectropolarimetric results for 3C~$279$ during two differing periods of flaring. Figure sourced from \citep{cooper_HEASA2022}.}
...
\begin{figure}[t]
    ...
    \caption{The reduced $Q$ and $U$ Stokes parameters for Hiltner~$652$, pre$-2006$ for {FORS1} \citep{FORS1}, $2010 - 2016$ for {FORS2} \citep{FORS2}, and post$-2022$ for \gls{SALT}. Figure sourced from \citep{cooper_HEASA2022}.}
    \label{fig:Hilt652}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{The photon flux of 3C~279. Figure created using data sourced from the Fermi Light Curve Repository \citep{FermiLCR}.\protect\footnotemark}
...
\todo{Discuss results from HEASA2022}
