...
The \stops\ \texttt{split} method accepts any \polsalt\ pre-reduced (`mxgbp-' prefixed) \gls{FITS} files as input and outputs \iraf\ compatible file structures. As no `split' \gls{FITS} files are created during pure \polsalt\ reductions, the \stops\ \texttt{split} method was tested by comparing the pre-reduced \polsalt\ files to the \texttt{split} methods output files, ensuring the correct structure and data integrity of the files handed off to \iraf.

\input{chapter_4/tables/4_split_info.tex}

\autoref{table:split_info} shows the \gls{FITS} file information for the files before and after splitting. The split `beam*.fits' files contain the split \gls{SCI} extension data as well as the same header as the pre-reduced file. The only changes to the data of each file is the exclusion of the opposing perpendicular polarization beam, as well as a cropping, which defaults to $40$~pixels, introduced to the top- and bottom-most rows of the data (see \autoref{subsec:stops_split}). This accounts for the discrepancy in the `Dimensions' between the input and output files in \autoref{table:split_info}.
