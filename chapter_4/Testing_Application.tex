...
Firstly, as the development of \stops\ was an iterative process, the software was continually checked to ensure compatibility such that the varying \stops\ method inputs were correctly parsed, and their outputs were parsable by the relevant \iraf\ tasks or \polsalt\ methods. As the \stops\ \texttt{split} and \texttt{join} methods are designed to convert between the \polsalt\ and \iraf\ file structures, much care was taken to ensure that the output of both methods provided accurate and consistent results.

Secondly, testing was performed throughout the development of \stops\ to ensure that no errant effects were introduced to the reduction process. Most notably the \stops\ \texttt{split} and \texttt{join} methods which are directly involved in the reduction process when implementing \stops.
...
The \stops\ \texttt{split} method had to accept any \polsalt\ pre-reduced \gls{FITS} files as input and output an \iraf\ compatible file structure.
...
The \texttt{correlate} method returns plots validating the wavelength solutions and so only had to accept the \polsalt\ \texttt{spectra extraction} method output files as input.
...
The \texttt{skylines} method returns plots validating the wavelength solutions and so only had to accept either the \iraf\ \texttt{transform} task or \stops\ \texttt{join} method output files as input.
