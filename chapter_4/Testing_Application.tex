...
\begin{figure}[t]
    ...
    \caption{The \stops\ \texttt{skylines} output for a synthetic 2D spectrum consisting of only noise, skyline emission features, and the characteristic \gls{RSS} chip gaps. Figure created using the \stops\ \texttt{skylines} method.}
    \label{fig:sky_test_pure}
\end{figure}

\todo{Discuss \autoref{fig:sky_test_pure}}

\begin{figure}[t]
    \centering
    \includegraphics[width = 1.0\textwidth]{4_sky_test_shift.pdf}
    \caption{The \stops\ \texttt{skylines} output for a two-dimensional spectrum, as described in \autoref{fig:sky_test_pure}, with a constant offset of $4$~\AA applied to all skyline emission features. Figure created using the \stops\ \texttt{skylines} method.}
    \label{fig:sky_test_shift}
\end{figure}

\todo{Discuss \autoref{fig:sky_test_shift}}
