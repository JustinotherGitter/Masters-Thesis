...
\todo{create subsubsections for split, iraf, join, and additionals (including transform)?}
...
\todo{Mention iraf tasks and interactive colon-commands here? This varies greatly based on user unlike pre-reductions/splitting/joining.}
\prgph
...
\todo{
    \begin{itemize}
        ...
        \item running skyline checks - skyline uses transformed images, check variation across the frame
    \end{itemize}
}

\todo{discuss transform here}
\begin{verbatim}cl> transform input output fitnames\end{verbatim}

\noindent where input is the same as the images pseudo-paramater name?????????????
