...
% Alternately, the \polsalt\ \gls{CLI} command:
% \begin{verbatim}\end{verbatim}
% \begin{lstlisting}[
%     language=bash,
%     label=code:polsalt_raw]
% $ python scripts/reducepoldata.py <.../>YYYYMMDD
% \end{lstlisting}
% then runs the raw image reduction process. It should be noted that the \polsalt\ \gls{CLI} command will, by default, attempt to run the wavelength calibration process. This behavior may be stopped by using the \texttt{-w} flag when running the command.
...
\todo{From appendix → Discuss - salt/py3 env, replace verbatim with listings for consistency, add polsalt GUI spectra extract, add polsalt GUI visualisation}
