...
\begin{verbatim}> python scripts/pol_extract.py c*.fits --yo A --ye B --dy spec_width\end{verbatim}

\noindent where the center row of the $O$ and $E$ beam spectra, \texttt{A} and \texttt{B}, as well as the spectral width, \texttt{spec\_width}, must be provided. These may be found through \gls{FITS} viewing software such as ds9\footnote{Available at \protect\url{https://sites.google.com/cfa.harvard.edu/saoimageds9}} or more manually through python.
\prgph
...
\noindent where a file name, which may also use a wildcard, is provided as well as optional para\-meters to define the binning (\texttt{bin=[int]A}), error bars (\texttt{errors=True|False}), choice of plot (\texttt{type=Ipt|Iqu}), save options (\texttt{save=[text][plot]}), and debug (\texttt{debug=False|True}).
